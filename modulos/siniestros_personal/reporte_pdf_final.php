...
try {
    ...
    $statsTipo = $pdo->query("SELECT tipo as categoria, COUNT(*) as total FROM siniestros_personal WHERE tipo IS NOT NULL AND tipo != '' GROUP BY tipo ORDER BY total DESC")->fetchAll(PDO::FETCH_ASSOC);
    $statsAseguradora = $pdo->query("SELECT aseguradora as categoria, COUNT(*) as total FROM siniestros_personal WHERE aseguradora IS NOT NULL AND aseguradora != '' GROUP BY aseguradora ORDER BY total DESC")->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <style>
        ...
        .chart-container-pie { position: relative; height: 220px; width: 100%; max-width: 100%; padding: 5px; box-sizing: border-box; }
        ...
        @media print {
            ...
            .chart-container-pie { height: 190px !important; }
        }
    </style>
</head>
...
<div class="container-fluid px-4">
    ...
    <div class="row g-3">
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header text-danger"><i class="fas fa-id-badge me-2"></i> DISTRIBUCIÓN POR TIPO (OFICIAL / CIVIL / COMISIONADO)</div>
                <div class="chart-container-pie"><canvas id="chartTipo"></canvas></div>
            </div>
        </div>
        ...
        <div class="col-md-6">
            <div class="card">
                <div class="card-header text-secondary"><i class="fas fa-shield-alt me-2"></i> DISTRIBUCIÓN POR ASEGURADORA</div>
                <div class="chart-container-pie"><canvas id="chartAseguradora"></canvas></div>
            </div>
        </div>
...
<script>
    ...
    // Gráfica de Tipos (Dona)
    new Chart(document.getElementById('chartTipo'), {
        type: 'doughnut',
        data: {
            labels: [<?php foreach($statsTipo as $s) echo "'".$s['categoria']."',"; ?>],
            datasets: [{
                data: [<?php foreach($statsTipo as $s) echo $s['total'].","; ?>],
    ...
    // Gráfica de Aseguradoras (Dona)
    new Chart(document.getElementById('chartAseguradora'), {
        type: 'doughnut',
        data: {
            labels: [<?php foreach($statsAseguradora as $s) echo "'".$s['categoria']."',"; ?>],
            datasets: [{
                data: [<?php foreach($statsAseguradora as $s) echo $s['total'].","; ?>],
                backgroundColor: ['#6f42c1', '#20c997', '#fd7e14', '#e83e8c', '#007bff', '#ffc107']
            }]
        },
        options: {
            responsive: true,
